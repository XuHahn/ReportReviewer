...
export default function ProjectGroups() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setUsers(u.users || [])
      setTags(t || [])
    } catch (e: any) { logger.error('Failed to load project groups', { component: 'ProjectGroups' }, e) }
    finally { setLoading(false) }
  }

  useEffect(() => {
    getCurrentUser().then(u => setCurrentUserId(u.employee_id)).catch((e: any) => { logger.error('Failed to get current user', { component: 'ProjectGroups' }, e) })
    fetchData()
  }, [])
  ...
  const filteredUsers = search.trim() ? users.filter(u => u.employee_id.includes(search) || (u.name || '').includes(search)) : users

  if (loading) return <div className="stats-loading"><div className="spinner"/> 加载中...</div>

  return (
    <div className="rules-manager">
      <div className="pg-toolbar">
        <h2 className="pg-title">项目组管理</h2>
        <button className="pg-btn" onClick={fetchData} style={{ marginRight: 8 }}>刷新</button>
        <button className="pg-add-btn" onClick={openNew}>+ 新建项目组</button>
      </div>

      {groups.length === 0 ? (
        <div className="pg-empty">暂无项目组，请点击"新建项目组"创建</div>
      ) : (
        <div className="pg-grid">
          ...
          <div key={g.id} className="pg-card">
            ...
            <div className="pg-card-members">
              {(g.members || []).map((eid: string) => {
                const u = users.find((x: any) => x.employee_id === eid)
                return (
                  <span key={eid} className="member-chip">
                    {u?.name || eid}
                    <span style={{ fontSize: 10, color: '#84a5d9', marginLeft: 4 }}>{eid}</span>
                    {eid === g.created_by && <span style={{ fontSize: 9, background: '#175cd3', color: '#fff', padding: '0 4px', borderRadius: 4, marginLeft: 4 }}>创建者</span>}
                  </span>
                )
              })}
            </div>
            <div className="pg-card-meta">{(g.members || []).length} 位成员</div>
          ...
          <div className="modal-container" onClick={e => e.stopPropagation()} style={{ maxWidth: 520 }}>
            ...
            <div className="modal-body" style={{ padding: 20 }}>
              ...
              <div className="pg-role-tabs">
                {['all', 'admin', 'reviewer', 'viewer'].map(r => (
                  <button key={r} className={`pg-role-tab ${roleFilter === r ? 'pg-role-tab-on' : ''}`} onClick={() => setRoleFilter(r)}>
                    {r === 'all' ? `全部 (${users.length})` : r === 'admin' ? `管理员 (${users.filter(u=>u.role==='admin').length})` : r === 'reviewer' ? `审核员 (${users.filter(u=>u.role==='reviewer').length})` : `查看者 (${users.filter(u=>u.role==='viewer').length})`}
                  </button>
                ))}
              </div>
              ...
              <div className="pg-member-list">
                {filteredUsers.filter(u => roleFilter === 'all' || u.role === roleFilter).map((u: any) => (
                  <label key={u.employee_id} className={`pg-member-item ${form.member_ids.includes(u.employee_id) ? 'pg-member-on' : ''}`}>
                    ...
                    <span className="pg-role-tag">{u.role === 'admin' ? '管理员' : u.role === 'reviewer' ? '审核员' : '查看者'}</span>
                  </label>
                ))}
                {filteredUsers.filter(u => roleFilter === 'all' || u.role === roleFilter).length === 0 && <div style={{ padding: 12, textAlign: 'center', color: '#98a2b3', fontSize: 13 }}>无匹配用户</div>}
              </div>
            </div>
