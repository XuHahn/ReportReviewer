...
export default function GroupsPage() {
  const queryClient = useQueryClient(); const [editing, setEditing] = useState<ProjectGroup | 'new' | null>(null); const [removeTarget, setRemoveTarget] = useState<ProjectGroup | null>(null)
  const groupsQuery = useQuery({ queryKey: ['groups'], queryFn: getProjectGroups })
  const usersQuery = useQuery({ queryKey: ['users'], queryFn: listUsers })
  async function remove() { if (!removeTarget) return; try { await deleteProjectGroup(removeTarget.id); notifications.show({ color: 'green', message: '项目组已删除' }); setRemoveTarget(null); queryClient.invalidateQueries({ queryKey: ['groups'] }) } catch (error) { notifications.show({ color: 'red', message: apiErrorMessage(error) }) } }
  return <div className="page groups-page"><PageHeader eyebrow="OWNERSHIP & SCOPE" title="项目组" description="分而治之，合而协同。" actions={<button className="primary-button" onClick={() => setEditing('new')}><Plus size={16}/>新建项目组</button>}/>
    ...
    <Modal opened={Boolean(editing)} onClose={() => setEditing(null)} title={editing === 'new' ? '新建项目组' : '编辑项目组'} size="lg" closeButtonProps={{ 'aria-label': '关闭项目组编辑窗口' }}><GroupForm group={editing === 'new' ? undefined : editing || undefined} users={usersQuery.data?.users || []} onDone={() => { setEditing(null); queryClient.invalidateQueries({ queryKey: ['groups'] }) }}/></Modal>
...
function GroupForm({ group, users, onDone }: { group?: ProjectGroup; users: Array<{ employee_id: string; name?: string }>; onDone: () => void }) {
