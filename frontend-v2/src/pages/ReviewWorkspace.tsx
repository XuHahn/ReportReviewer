...
import { shouldPollOverview } from '../reviewLogic'
...
export type ReviewStage = 'intake' | 'run' | 'findings' | 'complete'
const stages: Array<{ key: ReviewStage; index: string; label: string; hint: string }> = [
  { key: 'intake', index: '01', label: '资料与范围', hint: '四份必传资料' },
  { key: 'run', index: '02', label: '机器审核', hint: '证据图与规则' },
  { key: 'findings', index: '03', label: '问题裁决', hint: '人工结论' },
  { key: 'complete', index: '04', label: '审核完成', hint: '导出与修订' },
]
const versionDocTypes: DocType[] = ['order_form', 'test_plan', 'original_records', 'final_report']

export default function ReviewWorkspace() {
  const { setId = '', stage } = useParams<{ setId: string; stage: string }>()
  ...
  const overviewQuery = useQuery({
    queryKey: ['set', setId],
    queryFn: () => getDocumentSet(setId),
    enabled: Boolean(setId),
    refetchInterval: query => shouldPollOverview(query.state.data, current) ? 1500 : false,
  ...
  const runsQuery = useQuery({
    queryKey: ['runs', setId],
    queryFn: () => listReviewRuns(setId),
    enabled: Boolean(setId),
    refetchInterval: query => {
      const latestStatus = query.state.data?.runs?.[0]?.status
      return current === 'run' && ['created', 'building', 'running'].includes(latestStatus || '') ? 1500 : false
    },
  })
  const runId = runsQuery.data?.runs?.[0]?.graph_id
  const snapshotQuery = useQuery({ queryKey: ['snapshot', setId, runId], queryFn: () => getReviewRun(setId, runId!), enabled: current === 'findings' || current === 'complete' ? Boolean(runId) : false })
  ...
  }), [overview, latestRun, runsQuery.data, snapshotQuery.data, current])
  ...
  if (stage === 'extraction' && !overviewQuery.isLoading) {
  ...
  return <div className={`review-shell ${current === 'findings' ? 'findings-mode' : ''}`}>
    ...
    <nav className="stage-rail" aria-label="审核步骤">{stages.map(item => <button key={item.key} disabled={!stageAccess[item.key]} className={`${current === item.key ? 'active' : ''} ${stageStatus[item.key]}`} onClick={() => navigate(`/tasks/${setId}/${item.key}`)}>
      <span>{stageStatus[item.key] === 'done' ? '✓' : item.index}</span><div><b>{item.label}</b><small>{item.hint}</small></div>{item.key !== 'complete' && <i />}
    </button>)}</nav>
    <div className="review-stage">
      {current === 'intake' && <IntakeStage setId={setId} overview={overview} loading={overviewQuery.isLoading} />}
      {current === 'run' && <RunStage setId={setId} overview={overview} runs={runsQuery.data?.runs || []} />}
      {current === 'findings' && <FindingsStage setId={setId} snapshot={snapshotQuery.data} loading={snapshotQuery.isLoading} drawerOpen={drawerOpen} onDrawerClose={() => setDrawerOpen(false)} />}
      {current === 'complete' && <CompleteStage setId={setId} overview={overview} snapshot={snapshotQuery.data} />}
    </div>
    <Modal opened={versionsOpen} onClose={() => setVersionsOpen(false)} title="四份资料版本记录" size="lg" closeButtonProps={{ 'aria-label': '关闭版本记录窗口' }}><div className="version-history-list">{versionsQuery.isLoading ? <p>正在读取版本记录…</p> : versionsQuery.isError ? <p>版本记录读取失败，请稍后重试。</p> : versionDocTypes.map(type => { const state = versionsQuery.data?.[type]; return <section key={type}><h3>{DOC_LABELS[type]}</h3>{state?.failed ? <p className="version-load-error">该类资料的版本记录读取失败，请稍后重试。</p> : state?.versions.length ? state.versions.map(version => <div key={version.doc_id}><span>v{version.doc_version}</span><b>{version.filename}</b><small>{STATUS_LABELS[version.extraction_status || 'pending'] || version.extraction_status || '状态未记录'} · {version.created_at ? formatDate(version.created_at) : '时间未记录'}</small></div>) : <p>尚未上传</p>}</section> })}</div></Modal>
  </div>
}
