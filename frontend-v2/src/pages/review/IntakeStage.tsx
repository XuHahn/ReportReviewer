...
import { Archive, BookOpenCheck, Check, FileSpreadsheet, FileText, FolderArchive, RefreshCw, Search, ShieldCheck, Trash2, Upload, UsersRound, X } from 'lucide-react'
...
import { demoGroups, demoStandards } from '../../mockData'
...
const demoStandardReference: StandardReference = {
  reference_code: 'ISO 16750-2:2012',
  normalized_code: 'ISO1675022012',
  sources: ['test_plan', 'final_report'],
  evidence: ['引用示例：供电电压瞬时下降、Slow decrease and increase of the supply voltage'],
...
export default function IntakeStage({ setId, overview, loading, demo }: { setId: string; overview?: DocumentSetOverview; loading: boolean; demo: boolean }) {
  ...
  const standardsQuery = useQuery({ queryKey: ['standards', demo], queryFn: () => demo ? Promise.resolve(demoStandards) : getStandards() })
  const referencesQuery = useQuery({
    queryKey: ['standard-references', setId],
    queryFn: () => getDocumentSetStandardReferences(setId),
    enabled: standardOpen && !demo,
  })
  ...
  const reviewedDocs = new Set(docs.filter(doc => Boolean(doc.reviewed_at)).map(doc => doc.doc_type))
  const exceptionDocTypes = new Set<DocType>(extractionGate.blockers.map(item => item.doc_type).filter((type): type is DocType => slots.some(slot => slot.type === type)))
  const focusedReviewComplete = !autoGatePass && exceptionDocTypes.size > 0 && [...exceptionDocTypes].every(type => reviewedDocs.has(type))
  const complete = slots.every(slot => docs.some(doc => doc.doc_type === slot.type && doc.extraction_status === 'completed')) && (alreadyLocked || autoGatePass || focusedReviewComplete)
  ...
  useEffect(() => {
    ...
    const references = demo ? [demoStandardReference] : referencesQuery.data?.references
  ...
  }, [autoSelectionApplied, demo, referencesQuery.data, standardOpen, standardsQuery.data])
  useEffect(() => {
  ...
  async function upload(file: File, type: DocType, replaceId?: string) {
    try {
      if (!demo) {
        ...
        queryClient.setQueryData<DocumentSetOverview>(['set', setId, demo], current => {
        ...
        await queryClient.invalidateQueries({ queryKey: ['set', setId, demo] })
        ...
        await queryClient.invalidateQueries({ queryKey: ['sets', demo] })
      }
  ...
  async function remove() {
    ...
    try { if (!demo) await deleteDocument(setId, removeTarget.doc_id); setRemoveTarget(null); queryClient.invalidateQueries({ queryKey: ['set', setId, demo] }); queryClient.invalidateQueries({ queryKey: ['sets', demo] }); queryClient.invalidateQueries({ queryKey: ['stats', demo] }); notifications.show({ color: 'green', message: '文件已移除' }) }
  ...
  async function lockAndContinue() {
    ...
    try { if (!demo) await lockDocumentSet(setId, autoGatePass ? { auto_gate: true } : { exception_only: true }); queryClient.invalidateQueries({ queryKey: ['set', setId, demo] }); queryClient.invalidateQueries({ queryKey: ['sets', demo] }); queryClient.invalidateQueries({ queryKey: ['stats', demo] }); navigate(`/tasks/${setId}/run`) }
    catch (error) { notifications.show({ color: 'red', title: '暂时不能锁定', message: apiErrorMessage(error, '请完成四份资料的提取核查。') }) }
  }
  ...
  return <div className="stage-page intake-stage">
    ...
    <div className="intake-grid">
      ...
      <aside className="scope-panel"><div className="scope-title"><ShieldCheck/><div><h2>提交前硬门禁</h2><p>这些状态决定能否锁定。</p></div></div>
        ...
        <div className="scope-field"><div><UsersRound/><span><b>项目组 / 归属</b><small>{overview?.project_group_name || '用于权限、筛选和归档'}</small></span></div><Select aria-label="选择项目组" placeholder="选择" data={(groupsQuery.data || []).map(group => ({ value: group.id, label: group.name }))} value={overview?.project_group_id || null} onChange={async value => { if (value && !demo) await updateDocumentSetProjectGroup(setId, value); queryClient.invalidateQueries({ queryKey: ['set', setId, demo] }); queryClient.invalidateQueries({ queryKey: ['sets', demo] }) }} /></div>
        <button className="primary-button lock-button" disabled={!complete} onClick={lockAndContinue}>{alreadyLocked ? (overview?.status === 'reviewed' ? '审核已完成，查看结果' : '任务已锁定，进入机器审核') : complete ? (autoGatePass ? '通过门禁并开始审核' : '完成异常核查并开始审核') : autoGatePass ? `还需完成 ${slots.filter(slot => !docs.some(doc => doc.doc_type === slot.type && doc.extraction_status === 'completed')).length} 项` : `还需确认 ${[...exceptionDocTypes].filter(type => !reviewedDocs.has(type)).length} 份异常资料`}<Check size={16}/></button>
        {!complete && <p className="gate-help">{autoGatePass ? '资料完整后会自动跳过逐项核查。' : '提取异常不会被视为通过；请只处理异常资料。'}</p>}
      </aside>
    </div>
    <Modal opened={standardOpen} onClose={() => setStandardOpen(false)} title="配置本次审核的标准范围" size="xl" closeButtonProps={{ 'aria-label': '关闭标准选择窗口' }} classNames={{ content: 'standard-scope-modal', body: 'standard-scope-modal-body', header: 'standard-scope-modal-header' }}>
      <div className="standard-scope-shell">
        ...
        <div className="standard-scope-grid">
          <section className="standard-reference-panel"><header><div><h3>文档引用</h3><p>每个引用都要选用对应标准，或写明本轮跳过原因；原文中的标准编号与条款定位已用蓝色标出。</p></div><span>{demo ? '1 条' : referencesQuery.data ? `${referencesQuery.data.references.length} 条引用 · ${selectedStandards.length} 个标准` : '读取中'}</span></header><div className="standard-reference-scroll">{demo ? <StandardReferenceRow reference={demoStandardReference} standards={standardsQuery.data || []} selectedIds={selectedStandards} skipReason={skipReasons[demoStandardReference.normalized_code] || ''} onSkipReasonChange={value => setSkipReasons(current => ({ ...current, [demoStandardReference.normalized_code]: value }))} /> : referencesQuery.isLoading ? <div className="standard-scope-state">正在读取文档引用…</div> : referencesQuery.isError ? <div className="standard-scope-state reference-load-error">文档引用读取失败，暂时不能确认处理状态。</div> : referencesQuery.data?.references.length ? <>{referencesQuery.data.references.map(reference => <StandardReferenceRow key={reference.normalized_code} reference={reference} standards={standardsQuery.data || []} selectedIds={selectedStandards} skipReason={skipReasons[reference.normalized_code] || ''} onSkipReasonChange={value => setSkipReasons(current => ({ ...current, [reference.normalized_code]: value }))} />)}{extraSelectedStandards.length > 0 && <div className="standard-extra-selection"><header><b>本轮额外纳入标准</b><span>{extraSelectedStandards.length} 个</span></header><div>{extraSelectedStandards.map(standard => <em key={standard.id}>{standard.code}{standard.version ? ` · ${standard.version}` : ''}</em>)}</div><p>这些标准未在四份资料中明确引用，但会作为本轮审核的知识来源。</p></div>}</> : <div className="standard-scope-state">当前四份资料中未检测到明确的标准编号。</div>}</div></section>
